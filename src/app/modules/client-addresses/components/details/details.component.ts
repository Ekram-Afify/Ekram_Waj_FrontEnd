...
import {  AdminClientAdressDto, AdminClientAdressServiceProxy } from '@shared/service-proxies/service-proxies';
...
export class DetailsComponent extends AppComponentBase implements OnInit {
  ...
  customerAddress: AdminClientAdressDto = new AdminClientAdressDto();
  centers;
  isLinear = false;
  items: MenuItem[];
  ...
  ngOnInit() {
    this.items = [
      ...
      { label: this.l('CustomerAddressDetails') }
    ];
